...
Independent review found signed margin stress reversed bear/bull meaning for negative margins; fixed with margin +/- 10% of its absolute value and a red/green regression test. Forecast cash-flow table now uses calendar years. A live scan found 454 historical effective tax rates outside 0–100%; these factual reference values are now preserved and optional for pricing, while each forecast tax remains required within 0–100%. This isolated change was independently reviewed and tested. Final suite: 29 JavaScript tests and 20 Python tests passed.
...
Hosted verification: GitHub Actions manual refresh run 35241310514 completed successfully. Cloud collection attempted all 2,913 covered companies, returned 2,897 available forecasts and zero errors, persisted snapshots in commit 6d352a6 and published the site. 688256 cloud collection timestamp 2026-09-17T15:50:41Z; 600519 2026-09-17T15:46:07Z. Forecast stage takes roughly fifteen minutes from the hosted runner; the schedule is a start time, not a guaranteed completion time. Remote HTML and valuation modules were fetched and matched their local release versions. Browser interaction checks were completed on localhost; the browser tool timed out on remote navigation, so hosted content verification used HTTPS retrieval.
