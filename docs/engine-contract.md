...
Five explicit years use revenue growth and `FCFF = EBIT × (1-tax) + D&A - capex - deltaNWC`. Terminal FCFF is `year5 revenue × (1+g) × margin × (1-tax) × (1-g/ROIC)`. Zero growth has zero reinvestment even when terminal ROIC is zero. Enterprise value is explicit PV plus terminal PV; equity value adds cash and subtracts debt and minority interest, then divides by shares.
